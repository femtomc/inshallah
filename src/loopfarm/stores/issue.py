...
UNSUPPORTED_NODE_TAGS = ("node:control", "cf:sequence", "cf:fallback", "cf:parallel")
...
class IssueStore:
    ...
    def _issue_validation_findings(
        ...
        *,
        ...
        for issue_id in descendants:
            ...
            unsupported_node_tags = sorted(
                tag for tag in tags if tag in UNSUPPORTED_NODE_TAGS
            )
            if unsupported_node_tags:
                errors.append(
                    {
                        "code": "unsupported_node_tags",
                        "id": issue_id,
                        "tags": unsupported_node_tags,
                        "message": (
                            "unsupported node tags detected: "
                            "remove node:control/cf:* tags"
                        ),
                    }
                )
    ...
    def validate_dag(self, root_issue_id: str) -> dict[str, Any]:
        ...
        checks = {
            ...
            "unsupported_node_tags": not any(
                finding.get("code") == "unsupported_node_tags"
