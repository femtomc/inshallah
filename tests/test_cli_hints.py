"""Tests for contextual help, hints, and recovery messaging in loopfarm CLI."""
...
from loopfarm.cli import _run_parser, cmd_forum, cmd_init, cmd_issues, cmd_resume, cmd_run, cmd_status, main
from loopfarm.dag import DagResult
from loopfarm.issue_store import IssueStore
...
def test_issues_help_has_guide_cross_link(tmp_path: Path) -> None:
    ...
    assert rc == 0
    rendered = console.export_text()
    assert "loopfarm guide --section workflow" in rendered
...
def test_issues_unknown_subcommand_includes_recovery(tmp_path: Path, capsys) -> None:
    ...
    assert "loopfarm issues --help" in out["error"]
    assert "loopfarm guide --section workflow" in out["error"]
...
def test_forum_limit_error_includes_recovery(tmp_path: Path, capsys) -> None:
    ...
    assert "loopfarm forum read issue:<issue-id> --limit 20" in out["error"]
...
def test_status_shows_ready_next_steps(tmp_path: Path) -> None:
    ...
    rendered = console.export_text()
    assert "Next Steps" in rendered
    assert f"loopfarm issues get {issue['id']}" in rendered
    assert f"loopfarm forum read issue:{issue['id']} --limit 20" in rendered
    assert "loopfarm guide --section workflow" in rendered
...
def test_status_json_has_no_hint_pollution(tmp_path: Path, capsys) -> None:
    ...
    assert "loopfarm guide" not in raw
...
def test_run_missing_prompt_json_has_recovery(tmp_path: Path, capsys) -> None:
    ...
    assert "loopfarm run" in out["error"]
    assert "loopfarm guide --section workflow" in out["error"]
...
def test_run_json_emits_only_json_payload(tmp_path: Path, capsys) -> None:
    ...
    with (
        patch("loopfarm.cli._find_repo_root", return_value=tmp_path),
        patch("loopfarm.cli.DagRunner.run", return_value=DagResult(status="no_executable_leaf", steps=1, error="")),
    ):
        rc = cmd_run(args, Console())
...
def test_resume_json_emits_only_json_payload(tmp_path: Path, capsys) -> None:
    ...
    with (
        patch("loopfarm.cli._find_repo_root", return_value=tmp_path),
        patch("loopfarm.cli.DagRunner.run", return_value=DagResult(status="root_final", steps=2, error="")),
    ):
...
def test_resume_missing_root_json_has_recovery(tmp_path: Path, capsys) -> None:
    ...
    assert "loopfarm status" in out["error"]
    assert "loopfarm guide --section workflow" in out["error"]
